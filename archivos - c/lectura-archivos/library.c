...
int txtABinVar(char* linea,Empleado *emp)
{
    ...
    *act='\0';
    act = strrchr(linea,'\t');
    sscanf(act+1,"%d",&emp->sueldo);
    *act='\0';


    act = strrchr(linea,'\t');
    sscanf(act+1,"%2d/%2d/%4d",&emp->fNac.d,&emp->fNac.m,&emp->fNac.a);
    *act='\0';

    act = strrchr(linea,'\t');
    strcpy(emp->nomYApe,act+1);
    *act='\0';
...
int archivoTxtABin(const char* nombreTxt, const char* nombreBin)
{
    ...
    while(!errF && fgets(linea,TAM_LINEA,arcTxt))
    {
        ...
        if(strchr(linea,'\t')!=NULL)
            codRet = txtABinVar(linea,&empleadoTmp);
        else
